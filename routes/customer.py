...
def crtCustomer():
    ...
    try:
        ...
        if form.validate_on_submit():
            cstid = uuid.uuid4()
            cstname = (form.cstname.data).strip()
            cstlastname = (form.cstlastname.data).strip()
            cstphonenumber = (((form.cstphonenumber.data).strip()).replace("+57","")).replace(" ", "")
            ...
            return redirect("/customer")
        session['cstBackup'] = form.data
        flash("Ingrese los campos requeridos", "error")  
        return redirect("/customer")
